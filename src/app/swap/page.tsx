    ...
    <nav style={{position:'sticky',top:0,zIndex:100,borderBottom:'1px solid rgba(255,255,255,0.08)',background:'rgba(6,5,14,.92)',backdropFilter:'blur(20px)',padding:'0 2rem',display:'flex',alignItems:'center',justifyContent:'space-between',height:'64px'}}>
      ...
      <div style={{display:'flex',alignItems:'center',gap:'6px',flexWrap:'wrap'}}>
        {([['Home','/'],['Swap','/swap'],['Staking','/staking'],['Predictions','/predictions']] as const).map(([l,h])=>(
          <a key={l} href={h} style={{padding:'6px 14px',borderRadius:999,border:'1px solid rgba(255,255,255,0.08)',background:h==='/swap'?'rgba(255,255,255,0.1)':'rgba(255,255,255,0.03)',fontSize:'.8rem',fontWeight:500,color:'#e8e6f0',textDecoration:'none'}}>{l}</a>
        ))}
...
export default function SwapPage() {
  ...
  async function loadStats() {
    ...
    } catch (e: any) { setStatsErr(e?.message || 'Stats server not connected') }
  }
  ...
  async function onBuy() {
    ...
    if (wrongNetwork) { setTxError('Switch to Monad Mainnet (chain 143).'); return }
    ...
    <div style={{background:'#06050e',minHeight:'100vh'}}>
      ...
      <main className="mx-auto max-w-5xl px-4 py-10">
        <div className="mb-8">
          ...
          <p className="text-white/60 mt-2">Routes through fee router. <span className="text-white/80">1% buy fee → buyback wallet.</span></p>
        </div>
        {wrongNetwork && (
          <div className="mb-6 rounded-xl border border-yellow-400/30 bg-yellow-500/10 px-4 py-3 text-yellow-200 text-sm">
            ⚠️ Switch to <strong>Monad Mainnet (143)</strong> in your wallet.
          </div>
        )}
            ...
            <Card>
              ...
              <div className="grid grid-cols-2 gap-4 mt-5">
                <div>
                  ...
                  <input value={monIn} onChange={e => setMonIn(e.target.value)} className="w-full px-4 py-3 rounded-xl bg-black/30 border border-white/10 text-white outline-none" placeholder="0.1" inputMode="decimal" disabled={status==='pending'} />
                </div>
                <div>
                  <div className="text-white/70 text-sm mb-2">Slippage %</div>
                  <input value={slippage} onChange={e => setSlippage(e.target.value)} className="w-full px-4 py-3 rounded-xl bg-black/30 border border-white/10 text-white outline-none" placeholder="2" inputMode="decimal" disabled={status==='pending'} />
                </div>
              </div>
              <button onClick={onBuy} disabled={!isConnected || status==='pending' || wrongNetwork} className="mt-6 w-full rounded-xl py-3 font-semibold bg-white text-black hover:opacity-90 disabled:opacity-40 transition">
                {!isConnected ? 'Connect wallet to buy' : status==='pending' ? 'Buying…' : wrongNetwork ? 'Switch to Monad' : `Buy PHUCKMC with ${monIn||'0'} MON`}
              </button>
              <div className="mt-3 text-xs text-white/40">Fee router takes 1% and forwards the rest to nad.fun.</div>
              {status==='success' && txHash && <div className="mt-4 rounded-xl border border-emerald-400/30 bg-emerald-500/10 px-4 py-3 text-emerald-200 text-sm">✅ Confirmed! <a href={`https://monad-mainnet.blockscout.com/tx/${txHash}`} target="_blank" rel="noreferrer" className="underline">View →</a></div>}
              {txError && <div className="mt-4 rounded-xl border border-red-400/30 bg-red-500/10 px-4 py-3 text-red-300 text-sm">❌ {txError}</div>}
              {status==='pending' && txHash && <div className="mt-4 rounded-xl border border-white/10 bg-black/30 px-4 py-3 text-white/60 text-sm">⏳ Confirming… <a href={`https://monad-mainnet.blockscout.com/tx/${txHash}`} target="_blank" rel="noreferrer" className="underline text-white/80">{txHash.slice(0,16)}…</a></div>}
            ...
            <Card>
              ...
              {statsErr ? (
                <div className="rounded-xl border border-white/10 bg-black/30 p-4 text-white/50 text-sm">
                  ...
                  <div className="text-xs mt-1 text-white/35">Set <code>FEE_API_BASE</code> in Vercel env vars.</div>
                </div>
              ) : (
                <div className="space-y-3">
                  ...
                  <div className="rounded-xl border border-white/10 bg-black/30 p-4"><div className="text-white/60 text-sm">MON collected</div><div className="text-3xl font-bold text-white mt-1">{stats?.buybackMon ?? '—'}</div></div>
                </div>
              )}
            </Card>
            <Card>
              ...
              {[{ label: 'PHUCKMC', addr: TOKEN_ADDRESS }, { label: 'Fee Router', addr: FEE_ROUTER }].map(({ label, addr }) => (
                <div key={addr} className="mb-3 last:mb-0">
                  ...
                  <div className="rounded-lg border border-white/10 bg-black/30 px-3 py-2 text-white/70 text-xs font-mono break-all cursor-pointer hover:bg-white/5" onClick={() => navigator.clipboard.writeText(addr)}>{addr}</div>
                </div>
              ))}
            </Card>
          </div>
